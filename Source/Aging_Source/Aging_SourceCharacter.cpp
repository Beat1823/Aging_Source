...
#include "Aging_SourceCharacter.h"

AAging_SourceCharacter::AAging_SourceCharacter(const class FPostConstructInitializeProperties& PCIP)
	: Super(PCIP)
{
...
void AAging_SourceCharacter::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	// set up gameplay key bindings
	InputComponent->BindAction("Jump", IE_Pressed, this, &ACharacter::Jump);
	InputComponent->BindAxis("MoveRight", this, &AAging_SourceCharacter::MoveRight);

	InputComponent->BindTouch(IE_Pressed, this, &AAging_SourceCharacter::TouchStarted);
}

void AAging_SourceCharacter::MoveRight(float Value)
{
	// add movement in that direction
	AddMovementInput(FVector(0.f,-1.f,0.f), Value);
}

void AAging_SourceCharacter::TouchStarted(const ETouchIndex::Type FingerIndex, const FVector Location)
{
	// jump on any touch
	Jump();
}
